{
  ...
  environment.etc = {
    ...
    "fail2ban/action.d/ntfy.local".text = pkgs.lib.mkDefault (pkgs.lib.mkAfter ''
      ...
      actionban = ${pkgs.curl}/bin/curl -H "Title: <ip> has been banned in <name>" -d "<name> jail has banned <ip> from accessing $(hostname) after <failures> attempts." https://ntfy.sh/fail2ban-aorith
    '');
